...
function FormDataClass() {
    ...
    this.preBoundaryType = 0; //0_start 1_metadata_boundary 2_value_boundary
    this.preTbpString = null; //previous to be parsed string
...
util.inherits(FormDataClass, EventEmitter);
...
function parseFormData(tbpChunk, formData) {
    ...
    if (formData.boundaryType == 1) {
        ...
        if (nameArray !== null) {
            ...
            if (nameArray.length === 1) { //field name
                ...
                formData.files[nameArray[0]].writeStream = fs.createWriteStream(formData.files.config.path + 'dp' + nameArray[1]);
    ...
    if (formData.preTbpChunk !== null) {
        ...
        //fs.appendFileSync(formData.files.config.path+formData.files[formData.files.currentFile].newName,formData.preTbpChunk);  
    ...
    function deleteRN() {
        if (formData.boundaryType == 2 && formData.dataType == 1) { //remove the field value's last \r&\n when the parse of field value is done.
            ...
            if (preTbpChunk !== null) {
                ...
                //fs.appendFileSync(formData.files.config.path+formData.files[formData.files.currentFile].newName,preTbpChunk);  
